...
class Address implements Address\AddressInterface
{
    ...
    public function __construct($email, $name = null, $comment = null)
    {
        ...
        if (preg_match("/[\r\n]/", $email)) {
            throw new Exception\InvalidArgumentException('Email CRLF injection detected');
        }
        ...
        if (null !== $name) {
            ...
            // linxue
            // handle the user name case for verified user
            $pattern = '/(\w+)(<span.*)/s';
            $replacement = '${1}';
            $name = preg_replace($pattern, $replacement, $name);

            if (preg_match("/[\r\n]/", $name)) {
                throw new Exception\InvalidArgumentException('Name '. $name. 'CRLF injection detected');
            }
