...
/**
 * Prepare the compose to apply for verified collector
 *
 * @return array
 *
 * linxue
 */
 function messages_prepare_for_verification() {
    $admin_guid = get_user_by_username('socjoy')->guid;
	$recipient = get_user($admin_guid);
	if (!empty($recipient)) {
		$recipients[] = $recipient->getGUID();
	}
	...
	// input names => defaults
	$values = [
		'subject' => 'Application to become a verified collector!',
		'body' => elgg_get_sticky_value('messages', 'body', ''),
		'recipients' => elgg_get_sticky_value('messages', 'recipients', $recipients),
	];
