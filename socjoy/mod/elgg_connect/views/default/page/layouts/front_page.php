		...
		<div class="elgg-container groups">
			...
			foreach ($groups as $group) {
				...
				$image = elgg_format_element('div', [
					'class' => 'elgg-image',
				], elgg_view_entity_icon($group, 'medium'));
				...
				$title = elgg_format_element('h3', [], $url);

				$members = elgg_echo('members') . ": " . $group->getMembers(array('count' => true));
				$sub = elgg_format_element('div', [], $members);

				// linxue
				// align the pics with words
				$body = elgg_format_element('div', ['class' => 'elgg-body'], $image. $title . $sub);

				echo elgg_format_element('div', ['class' => 'elgg-image-block'], $body);
